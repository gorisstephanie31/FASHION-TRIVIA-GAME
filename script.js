// =============================================
// FASHION DATA — FROM YOUR VOGUE ARCHIVE CSV
// =============================================
const fashionData = [
...
// Get all unique designer names for wrong answer pool
...
function generateChoices(correctDesigner) {
  const wrong = allDesigners.filter(d => d !== correctDesigner);
  const shuffledWrong = shuffle(wrong).slice(0, 3);
  return shuffle([correctDesigner, ...shuffledWrong]);
}

// Load a question
function loadQuestion() {
  answered = false;
  const item = shuffledData[currentQuestion];
  const designer = item[0];
  const imageUrl = item[2];

  document.getElementById('runway-image').src = imageUrl;
  document.getElementById('feedback').textContent = '';
  document.getElementById('next-btn').style.display = 'none';

  const choices = generateChoices(designer);
  ...
  choices.forEach(choice => {
    ...
    btn.addEventListener('click', () => handleAnswer(btn, choice, designer));
    container.appendChild(btn);
  });
}

// Handle answer selection
function handleAnswer(selectedBtn, selectedDesigner, correctDesigner) {
  if (answered) return;
  answered = true;
  ...
  allBtns.forEach(btn => {
    btn.disabled = true;
    if (btn.textContent === correctDesigner) {
      btn.classList.add('correct');
    }
  });

  if (selectedDesigner === correctDesigner) {
    score++;
...
function showScore() {
  ...
  document.getElementById('final-message').textContent = `WELL DONE, ${playerName.toUpperCase()}!`;
...
// Start button → Name page
document.getElementById('start-btn').addEventListener('click', () => {
  showPage('name-page');
});

// Name submit → Game page
document.getElementById('name-submit-btn').addEventListener('click', () => {
  ...
  score = 0;
  currentQuestion = 0;
  shuffledData = shuffle(fashionData);
